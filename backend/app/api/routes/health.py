...
async def status(c: Container = Depends(container)) -> SystemStatus:
    ...
    services = [
        ...
        ServiceStatus(name="vosk", healthy=vosk_ok, detail=c.settings.vosk_api_url),
        ServiceStatus(name="ocr", healthy=ocr_ok,
                      detail="Tesseract available" if ocr_ok else "Tesseract not found"),
